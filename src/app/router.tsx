...
export const rootRoute = createRootRoute({
  component: () => (
    <Suspense fallback="Échec de chargement — relance idols launcher.">
      <Frame />
    </Suspense>
  ),
...
export const onboardRoute = createRoute({
  ...
  component: Onboard,
  beforeLoad: () => {
    if (!useUserControl.getState().access_token?.trim()) {
      ensureAuthLayout();
      throw redirect({ to: "/credentials" });
    }
    ensureMainLayout();
